...
def progress(streams, chunk: bytes, bytes_remaining: int):
    ...
    print(
        "\r"
        + "[Download progress]:[%s%s]%.2f%%;"
        % (
            "█" * int(size * 20 / contentsize),
            " " * (20 - int(size * 20 / contentsize)),
            float(size / contentsize * 100),
        ),
        end="",
    )
...
def download():
    ...
    # print(RES)
    if not url.get():
        messagebox.showinfo("Error", "Enter a download link")
        return
            ...
            if language:
                subprocess.run(
                    [
                        "ffmpeg",
                        "-i",
                        download_Folder + "video.mp4",
                        "-i",
                        download_Folder + "audio.mp3",
                        "-i",
                        download_Folder + "subs.srt",
                        "-c:v",
                        "copy",
                        "-c:a",
                        "copy",
...
def videodetails():
    link = url.get()
    if not link:
        messagebox.showinfo("Error", "Enter a link")
        return
    try:
        yt = pytube.YouTube(link, on_progress_callback=progress)

    except:
        messagebox.showinfo("Error", "Enter a valid link")
        return
    ...
    resolutions = [
        (q, s) for q, s in sorted(resolutions.items(), key=lambda x: (len(x[0]), x[0]))
    ]
    new = []
    ...
    for choice in resolutions:
        qualitySelect["menu"].add_command(
            label=choice, command=tk._setit(clicked, choice)
        )
...
# labels
label1 = tk.Label(root, text="Youtube link: ", padx=10, pady=10)

label2 = tk.Label(root, text="Destination: ", padx=10, pady=5)

title = tk.Label(root, text="Title:", padx=10, pady=10)
author = tk.Label(root, text="Author:", padx=10, pady=10)
publishDate = tk.Label(root, text="Published date:", padx=10, pady=10)
views = tk.Label(root, text="Number of views:", padx=10, pady=10)
duration = tk.Label(root, text="Length of video:", padx=10, pady=10)

text = tk.StringVar()
downloadProgress = tk.Label(textvariable=text)

# buttons
browse = tk.Button(root, text="Browse", command=browse, width=10)

download = tk.Button(root, text="Download", command=download, bg="#567", fg="White")
...
label1.grid(row=2, column=0, pady=5, padx=5)

url.grid(row=2, column=1, pady=5, padx=5, columnspan=2)

getDetails.grid(row=2, column=3, pady=20, padx=20)

label2.grid(row=3, column=0, pady=5, padx=5)

dest.grid(row=3, column=1, pady=5, padx=5)

browse.grid(row=3, column=2, pady=1, padx=1)

qualitySelect.grid(row=4, column=3)

download.grid(row=5, column=3)
